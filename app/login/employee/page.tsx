...
export default function EmployeeLoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      console.log('Attempting login with:', { email: normalizedEmail, password: '***', portal: 'employee' });
      const authResponse = await validateCredentials('employee', normalizedEmail, password);
      console.log('Auth response:', authResponse);
      
      if (authResponse.success && authResponse.session) {
        console.log('Login successful, storing session...');
        storeSession(authResponse.session);
        router.push(authResponse.redirectTo || '/employee/dashboard');
      } else {
