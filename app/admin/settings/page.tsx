...
export default function Settings() {
  ...
  // 👇 SIMPLE TAX FIELD - Sirf ek field
  const [taxPercentage, setTaxPercentage] = useState(5) // Default 5%
          ...
          if (data.quotationDefaults) {
            setQuotationDefaults({
              ...
              bankDetails: {
                ...
                ...(data.quotationDefaults.bankDetails || {})
              }
            })
          }

          // 👇 SET TAX PERCENTAGE - Simple field
          if (data.taxPercentage !== undefined) {
            setTaxPercentage(data.taxPercentage)
          } else if (data.tax && data.tax.rate) {
            // For backward compatibility
            setTaxPercentage(data.tax.rate)
          }
        }
      } catch (error) {
  ...
  const handleSaveSettings = async () => {
    ...
    try {
      ...
      // 👇 SIMPLE - Save to tax collection as well
      await setDoc(doc(db, 'tax', 'default'), {
        rate: taxPercentage,
        updatedAt: new Date()
      })
  ...
  const handleQuotationDefaultsChange = (
    field:
      | 'notes'
      | 'terms'
      | 'confirmationLetter'
    ...
    setQuotationDefaults({ ...quotationDefaults, [field]: value })
    setShowSave(true)
  }
  ...
  const handleCompanySealUpload = (file: File | null) => {
    ...
    reader.onload = () => {
      ...
      setQuotationDefaults({ ...quotationDefaults, companySealImage: result })
      setShowSave(true)
    }
    reader.readAsDataURL(file)
  }

  const handleQuotationBankDetailChange = (
    field: keyof typeof DEFAULT_QUOTATION_BANK_DETAILS,
    value: string
  ) => {
    setQuotationDefaults({
      ...quotationDefaults,
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
        ...
        <div className="lg:col-span-3 space-y-6">
          ...
          {activeSection === 'quotation' && (
            <div className="bg-card rounded-xl border shadow-sm p-6 space-y-6">
              <h2 className="text-xl font-bold">Quotation Defaults</h2>
              <p className="text-sm text-muted-foreground">
                These values will auto-fill when creating a new quotation.
              </p>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-1.5 md:col-span-2">
                  <label className="block text-sm font-medium">Confirmation Letter</label>
                  <textarea
                    rows={4}
                    value={quotationDefaults.confirmationLetter}
                ...
                <div className="space-y-1.5 md:col-span-2">
                  <label className="block text-sm font-medium">Company Seal/Signature Image (for PDF)</label>
                  <input
                    type="file"
                    ...
                    className="w-full px-4 py-2 bg-muted border rounded-lg focus:outline-none focus:ring-2 focus:ring-pink-500"
                  />
                  {quotationDefaults.companySealImage && (
                    <div className="border rounded-lg p-3 bg-white">
                      <img
                        src={quotationDefaults.companySealImage}
                        alt="Company seal preview"
                        className="h-20 object-contain"
                      />
                      <button
                        type="button"
                        onClick={() => handleQuotationDefaultsChange('companySealImage', '')}
                  ...
                  <p className="text-xs text-muted-foreground">
                    This image will appear in the company signature box on the Confirmation & Approval page.
                  </p>
                </div>
                ...
                <div className="space-y-1.5">
                  ...
                  <textarea
                    rows={4}
                    value={quotationDefaults.notes}
                    onChange={(e) => handleQuotationDefaultsChange('notes', e.target.value)}
                    className="w-full px-4 py-2 bg-muted border rounded-lg focus:outline-none focus:ring-2 focus:ring-pink-500 resize-none"
                    placeholder="Default payment terms for quotations"
                  />
                </div>
                ...
                <div className="space-y-1.5">
                  ...
                  <textarea
                    ...
                    onChange={(e) => handleQuotationDefaultsChange('terms', e.target.value)}
                    className="w-full px-4 py-2 bg-muted border rounded-lg focus:outline-none focus:ring-2 focus:ring-pink-500 resize-none"
                ...
                <div>
                  ...
                  <input
                    type="text"
                    value={quotationDefaults.bankDetails.accountName}
                    onChange={(e) => handleQuotationBankDetailChange('accountName', e.target.value)}
                    className="w-full px-4 py-2 bg-muted border rounded-lg focus:outline-none focus:ring-2 focus:ring-pink-500"
                ...
                <div>
                  ...
                  <input
                    type="text"
                    value={quotationDefaults.bankDetails.accountNumber}
                    onChange={(e) => handleQuotationBankDetailChange('accountNumber', e.target.value)}
                    className="w-full px-4 py-2 bg-muted border rounded-lg focus:outline-none focus:ring-2 focus:ring-pink-500"
                ...
                <div>
                  ...
                  <input
                    type="text"
                    value={quotationDefaults.bankDetails.bankName}
                    onChange={(e) => handleQuotationBankDetailChange('bankName', e.target.value)}
                    className="w-full px-4 py-2 bg-muted border rounded-lg focus:outline-none focus:ring-2 focus:ring-pink-500"
                ...
                <div>
                  ...
                  <input
                    type="text"
                    value={quotationDefaults.bankDetails.swiftCode}
                    onChange={(e) => handleQuotationBankDetailChange('swiftCode', e.target.value)}
                    className="w-full px-4 py-2 bg-muted border rounded-lg focus:outline-none focus:ring-2 focus:ring-pink-500"
                ...
                <div className="md:col-span-2">
                  ...
                  <input
                    type="text"
                    value={quotationDefaults.bankDetails.iban}
                    onChange={(e) => handleQuotationBankDetailChange('iban', e.target.value)}
                    className="w-full px-4 py-2 bg-muted border rounded-lg focus:outline-none focus:ring-2 focus:ring-pink-500"
          ...
          {/* 👇 SIMPLE TAX SETTINGS - Sirf ek field */}
          {activeSection === 'tax' && (
            <div className="bg-card rounded-xl border shadow-sm p-6 space-y-6">
              <div className="flex items-center gap-3 border-b pb-4 mb-2">
                <Percent className="h-6 w-6 text-pink-600" />
                <h2 className="text-xl font-bold">Tax Applied on Client Services</h2>
              </div>
                ...
                <div className="flex items-center gap-4">
                  <div className="relative flex-1">
                    <input
                      ...
                      className="w-full px-4 py-3 bg-white border-2 border-pink-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-pink-500 text-lg font-bold"
                      placeholder="5"
                      min="0"
                      max="100"
                      step="0.1"
                    />
                    <span className="absolute right-4 top-1/2 -translate-y-1/2 text-pink-600 font-bold">%</span>
                  </div>
                  <div className="bg-pink-100 px-4 py-3 rounded-lg">
                    <span className="text-pink-800 font-bold">{taxPercentage}% Tax</span>
                  </div>
                </div>
                <p className="text-xs text-muted-foreground mt-3">
                  This tax percentage will be applied to all client services and quotations
                </p>
                ...
                <div className="mt-6 p-4 bg-gray-50 rounded-lg border">
                  ...
                  <div className="space-y-1 text-sm">
                    ...
                    <div className="flex justify-between text-pink-600">
                      ...
                      <span>AED {taxPercentage * 10}</span>
                    </div>
                    <div className="border-t pt-1 mt-1 flex justify-between font-bold">
                      <span>Total:</span>
                      <span>AED {1000 + (taxPercentage * 10)}</span>
                    </div>
                  </div>
                </div>
              </div>
      ...
      {showSave && (
        <div className="fixed bottom-6 right-6 z-50">
          <button
            ...
            disabled={isSaving}
            className="px-6 py-3 bg-pink-600 text-white rounded-lg text-sm font-bold hover:bg-pink-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2 shadow-lg"
          >
            {isSaving ? (
              <>
