...
type SurveyAnswer = string | number | string[] | null
...
export default function SurveyPage() {
    ...
    const fetchSurvey = async () => {
      ...
      try {
        ...
        if (surveyDoc.exists()) {
          ...
          const sourceSections = Array.isArray(data.sections) ? (data.sections as SurveySection[]) : []
          const processedSections = sourceSections.map((section) => ({
            ...section,
  ...
  const handleSubmit = async () => {
    ...
    try {
      const responses: SurveyResponseItem[] = []
        ...
        if (section.questions && Array.isArray(section.questions)) {
          section.questions.forEach((question) => {
            ...
            if (answer !== undefined && answer !== null && answer !== '') {
              const isImageUpload = question.type === 'image-upload'
              const imageAnswer = Array.isArray(answer)
              ...
              responses.push({
                ...
                answer: isImageUpload ? 'Images uploaded' : answer,
                // For image upload, store count only to save space
                imageCount: isImageUpload ? imageAnswer.length : undefined,
                // Optionally store first image as thumbnail
                thumbnail: isImageUpload && imageAnswer.length > 0 ? imageAnswer[0] : undefined
              })
            }
          })
        }
      ...
      if (errMessage.includes('size') || errCode === 'resource-exhausted') {
  ...
  const ImageUploadInput = ({ question }: { question: SurveyQuestion }) => {
    ...
    return (
      <div className="space-y-3">
        ...
        <div className="text-xs text-gray-400 flex flex-wrap gap-2">
          {acceptedFormats.map((format, idx: number) => (
  ...
  const renderQuestionInput = (question: SurveyQuestion) => {
    ...
    switch(question.type) {
      ...
      case 'rating':
        ...
        const numericRating = typeof value === 'number' ? value : 0
        return (
          <div className="space-y-2">
            <div className="flex gap-1">
              {Array.from({ length: ratingMax }, (_, i) => i + 1).map((star) => (
                <button
                  ...
                  className={`text-3xl ${numericRating >= star ? 'text-yellow-400' : 'text-gray-300'} hover:text-yellow-300`}
                >
                  ★
                </button>
              ))}
      ...
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="text-center">
          <h2 className="text-2xl font-bold text-gray-900">Survey Not Found</h2>
              <p className="mt-2 text-gray-600">The survey you are looking for does not exist or has been removed.</p>
        </div>
      </div>
    )
  }
  ...
  const totalQuestions = survey.sections?.reduce((total: number, section) => {
        ...
        <div className="bg-white rounded-lg shadow-lg overflow-hidden">
          ...
          {totalQuestions === 0 ? (
            ...
            <form onSubmit={(e) => { e.preventDefault(); handleSubmit(); }} className="p-6">
              {survey.sections?.map((section, sectionIndex: number) => {
                ...
                return (
                  <div key={section.id || sectionIndex} className="mb-8 last:mb-0">
                    ...
                    <div className="space-y-6">
                      {sectionQuestions.map((question, questionIndex: number) => (
