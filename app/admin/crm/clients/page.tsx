...
export default function ClientProfiles() {
  ...
  const [servicesMap, setServicesMap] = useState<Map<string, string>>(new Map()) // id -> name mapping

  // Tax rate from Firebase
  ...
  // 👇 NEW - Fetch services from Firebase
  useEffect(() => {
    ...
    const fetchTaxRate = async () => {
      try {
        ...
        if (taxDoc.exists()) {
          ...
          setTaxRate(taxData.rate || 5)
          console.log('Tax rate loaded:', taxData.rate)
        } else {
          console.log('No tax document found, using default 5%')
          setTaxRate(5)
        }
      } catch (error) {
  ...
  // 👇 NEW - Function to get service name from ID
  const getServiceNameFromId = (serviceId: string): string => {
    if (!serviceId || serviceId === 'No service yet') return serviceId || 'No service yet'
    
    // Check if it looks like a Firebase ID (usually 20+ chars with letters/numbers)
    if (serviceId.length > 15 && /^[A-Za-z0-9]+$/.test(serviceId)) {
      const serviceName = servicesMap.get(serviceId)
      if (serviceName) {
        return serviceName
      }
      // If not found in map, return a default name
      return 'Service Completed'
    }
    
    // If it's not an ID format, return as is
    return serviceId
  }
  ...
  const metrics = useMemo(() => {
    ...
    const totalWithTax = totalRevenue + totalTax
    
    const avgClientValue = clients.length > 0 ? totalRevenue / clients.length : 0
    const avgTaxPerClient = clients.length > 0 ? totalTax / clients.length : 0
        ...
        <div className="bg-white p-4 rounded-lg border border-gray-200">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-pink-600">{metrics.taxRate}%</p>
            </div>
        ...
        <div className="bg-white p-4 rounded-lg border border-gray-200">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-orange-600">AED {metrics.totalTax.toLocaleString()}</p>
            </div>
              ...
              <table className="w-full">
                ...
                <tbody className="divide-y divide-gray-200">
                  {paginatedClients.map((client) => {
                    const clientTax = client.totalSpent * (taxRate / 100)
                    const clientTotalWithTax = client.totalSpent + clientTax
                    ...
                    return (
                      <tr key={client.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-4 py-4 whitespace-nowrap text-sm text-pink-600 font-medium">
                          AED {clientTax.toLocaleString()}
                        </td>
                        <td className="px-4 py-4 whitespace-nowrap text-sm text-purple-600 font-bold">
                          AED {clientTotalWithTax.toLocaleString()}
                        </td>
                        <td className="px-4 py-4 whitespace-nowrap text-sm text-gray-900">
                          <div className="flex items-center gap-1">
                            <Clock className="h-3 w-3 text-gray-400" />
                            <span title={`Service ID: ${client.lastService}`}>
                              {getServiceNameFromId(client.lastService)}
                            </span>
                          </div>
                        </td>
          ...
          <div className="bg-white rounded-lg max-w-4xl w-full max-h-[90vh] overflow-hidden">
            ...
            <div className="p-8 grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <div className="space-y-6">
                <div className="bg-gray-50 border border-gray-300 rounded-2xl p-6">
                  <h4 className="text-sm font-black text-gray-600 uppercase tracking-widest mb-4">Financial Summary</h4>
                  <div className="space-y-3">
                    <div className="flex justify-between items-center pb-2 border-b border-gray-200">
                      <span className="text-sm text-gray-600">Total Spent:</span>
                      <span className="font-bold text-gray-900">AED {selectedClient.totalSpent.toLocaleString()}</span>
                    </div>
                    <div className="flex justify-between items-center pb-2 border-b border-gray-200">
                      ...
                      <span className="font-bold text-pink-600">AED {(selectedClient.totalSpent * taxRate / 100).toLocaleString()}</span>
                    </div>
                    <div className="flex justify-between items-center pt-2">
                      <span className="text-sm font-bold text-gray-800">Total with Tax:</span>
                      <span className="font-bold text-lg text-purple-700">
                        AED {(selectedClient.totalSpent * (1 + taxRate/100)).toLocaleString()}
                      </span>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-sm font-black text-gray-900">
                        {getServiceNameFromId(selectedClient.lastService)}
                      </p>
